...
class Tenant extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('read_only', 'numerical', 'integerOnly'=>true),
			array('tenant_id, name', 'length', 'max'=>256),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, tenant_id, name, read_only', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'promoteCodes' => array(self::HAS_MANY, 'PromoteCodes', 'tenant_id'),
		);
	}
